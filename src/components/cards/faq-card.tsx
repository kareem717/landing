	...
	return (
		<div>
			...
			>
				<CardHeader
					className={cn(
						"flex flex-row items-center justify-between font-medium gap-2"
					)}
				...
				>
					...
					<ChevronDownCircle
						className={cn(
							"shrink-0 transition duration-200 w-6 h-6 sm:h-[min(2vw,90px)] sm:w-[min(2vw,90px)]",
							isOpen && "rotate-180"
						)}
					/>
				</CardHeader>
				<CardContent
					className={cn(
						...
						isOpen ? "opacity-100 max-h-[25vh]" : "opacity-0 max-h-0 " // TODO: need to get rid of hidden and make this smooth
					)}
				>
					<p>{desc}</p>
				</CardContent>
